...
[Sqli-Lab](!https://github.com/Audi-1/sqli-labs)是印度一个小哥弄出来练习SQL注入教程，但是因为项目是几年前的，在目前的PHP7环境下已经无法运行,后来我找了另外一个修改后可以使用的[版本,Sqli_Edited_Version](!https://github.com/Rinkish/Sqli_Edited_Version).
...
11. Less-11- Error Based- String
    ...
    但是其实这个题目还可以通过报错注入得到结果，以下是过程：
        ...
        ```
        id=1' and (select 1 from (select count(*),concat((payload),floor(rand(0)*2))a from information_schema.tables group by a)x) 
        ```
        第二个是使用`updatexml()`
        ```
